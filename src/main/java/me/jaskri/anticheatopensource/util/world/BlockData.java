package me.jaskri.anticheatopensource.util.world;
...
public enum BlockData {
   ...
   /* _RAIL((protocol, b) -> ReflectionUtil2.getBlockBoundingBox(b).toCollisionBox(),Arrays.stream(Material.values())
    ...
    ,_WALL(new DynamicWall(), Arrays.stream(XMaterial.values())
            ...
            .map(me.jaskri.anticheatopensource.base.user.objects.BlockData::m)
            .toArray(Material[]::new)),
    ...
    _NONE2(NoCollisionBox.INSTANCE, Arrays.stream(XMaterial.values())
            .filter(mat -> {
                ...
                return names.stream().anyMatch(name ->
                        name.contains("PLATE"));
            }).map(me.jaskri.anticheatopensource.base.user.objects.BlockData::m).toArray(Material[]::new)),
    ...
    private static me.jaskri.anticheatopensource.base.user.objects.BlockData[] lookup = new me.jaskri.anticheatopensource.base.user.objects.BlockData[Material.values().length];

    static {
        for (me.jaskri.anticheatopensource.base.user.objects.BlockData data : values()) {
            for (Material mat : data.materials) lookup[mat.ordinal()] = data;
        }
    }

    public static me.jaskri.anticheatopensource.base.user.objects.BlockData getData(Material material) {
        Material matched = MiscUtils.match(material.toString());
        me.jaskri.anticheatopensource.base.user.objects.BlockData data = lookup[matched.ordinal()];
